...
func verifyPathIsSafeForExec(execPath string) (string, error) {
	unsafe, unsafeErr := regexp.MatchString(ExecPathBlackListRegex, execPath)
	_, statError := os.Stat(execPath)
	if unsafe {
		return "", errors.Wrap(unsafeErr, "Failed to determine is path is safe to execute: "+execPath)
	} else if statError != nil {
		return "", errors.Wrap(statError, "Failed to determine is path is safe to execute: "+execPath)
	}
	return execPath, nil
}
