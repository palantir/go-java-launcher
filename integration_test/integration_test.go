...
func TestMainMethod(t *testing.T) {
	...
	assert.Regexp(t, `Argument list to executable binary: \[.+/bin/java -Xmx4M -Xmx1g -classpath .+/github.com/palantir/go-launcher/integration_test/test_resources Main arg1\]`, output)
...
func TestMainMethodWithoutCustomConfig(t *testing.T) {
	...
	assert.Regexp(t, `Argument list to executable binary: \[.+/bin/java -Xmx4M -classpath .+/github.com/palantir/go-launcher/integration_test/test_resources Main arg1\]`, output)
	// expected output of Java program
	assert.Regexp(t, `\nmain method\n`, string(output))
}
